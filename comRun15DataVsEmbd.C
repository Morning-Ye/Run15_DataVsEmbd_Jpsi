	...
	for(int i=0; i<nVar; i++){
		...
		for(int j=0; j<nPtBin; j++){
			...
			if(primeonly_flag) {
				...
				HistDataPrime_SIG_PT[i][j]->SetMarkerStyle(21);
				HistDataPrime_SIG_PT[i][j]->SetMarkerColor(2);
				HistDataPrime_SIG_PT[i][j]->SetLineColor(2);
				HistDataPrime_SIG_PT[i][j]->SetTitle(dataHistName[i]);
				
				int tembin = HistDataPrime_SIG_PT[i][j]->GetMaximumBin();
				HistDataPrime_SIG_PT[i][j]->SetMaximum(HistDataPrime_SIG_PT[i][j]->GetBinContent(tembin)*1.6);
				...
				tl.DrawLatex(0.65, 0.4, Form("%.1f<pt<%.1f",ptlow_prime[j], pthigh_prime[j]));
			}
			if(partonly_flag){
				...
				HistDataPart_SIG_PT[i][j]->SetMarkerStyle(21);
				HistDataPart_SIG_PT[i][j]->SetMarkerColor(8);
				HistDataPart_SIG_PT[i][j]->SetLineColor(8);
				int tembin = HistDataPart_SIG_PT[i][j]->GetMaximumBin();
				HistDataPart_SIG_PT[i][j]->SetMaximum(HistDataPart_SIG_PT[i][j]->GetBinContent(tembin)*1.6);
				...
				HistEmbd_SIG_PT[i][j]->SetLineColor(1);
				HistEmbd_SIG_PT[i][j]->Draw("histsame");
				tl.DrawLatex(0.65, 0.4, Form("%.1f<pt<%.1f",ptlow_partner[j], pthigh_partner[j]));
			}
			
			TLegend *leg = new TLegend(0.65, 0.75, 0.89, 0.89);
			if(primeonly_flag){
				leg->AddEntry(HistDataPrime_SIG_PT[i][j],"Data Trig e", "lp");
				leg->AddEntry(HistEmbdTrg_SIG_PT[i][j], "Embd Trig e", "lp");
			}
			if(partonly_flag){
				leg->AddEntry(HistDataPart_SIG_PT[i][j], "Data notTrig e", "lp");
				leg->AddEntry(HistEmbd_SIG_PT[i][j], "Embd notTrig e", "lp");
			}
			leg->Draw("same");
